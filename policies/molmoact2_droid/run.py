...
"""Evaluate MolmoAct2-DROID across registered RoboLab tasks."""
...
POLICY = "molmoact2_droid"

parser = argparse.ArgumentParser(description="Evaluate the MolmoAct2-DROID policy backend.")
...
parser.add_argument(
    ...
    "--remote_uri",
    default=None,
    help="Full HTTP(S) server URI. It may include the /act endpoint.",
)
parser.add_argument(
    "--open-loop-horizon",
    "--open_loop_horizon",
    default=None,
    type=int,
    help="Actions consumed from each 15-step chunk (default: 15).",
)
...
from policies.molmoact2_droid.client import MolmoAct2DroidClient  # noqa: E402
...
def make_client(args: argparse.Namespace) -> MolmoAct2DroidClient:
    kwargs = {
    ...
    return MolmoAct2DroidClient(**{key: value for key, value in kwargs.items() if value is not None})
